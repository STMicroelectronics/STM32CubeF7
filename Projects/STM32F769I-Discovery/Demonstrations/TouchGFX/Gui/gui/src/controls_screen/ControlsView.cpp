...
void ControlsView::setupScreen()
{
    ...
    headline.setPosition(200, 20, 400, 30);
    add(headline);
    ...
    JunctionTempValue.setXY(443, 13);
#else
    board.setBitmap(Bitmap(BITMAP_EVAL_BOARD_IMAGE_ID));
    board.setXY(35, 45);
...
#ifdef USE_STM32769I_DISCO
    JunctionTempScreen.setWidth(swipeContainer.getWidth());
    JunctionTempScreen.setHeight(swipeContainer.getHeight());
    JunctionTempScreen.add(board);
    JunctionTempScreen.add(JunctionTempValue);
    JunctionTempScreen.add(line);
    swipeContainer.add(JunctionTempScreen);
#else
...
#ifdef USE_STM32769I_DISCO
void ControlsView::setJunctionTempValue(int value)
{
    int newPercentage = value; //(value * 100) / 127;
    if (newPercentage != JunctionTempValue.getValue())
    {
        JunctionTempValue.setValue(newPercentage);
    }
}
#else
void ControlsView::setPotentiometerValue(int value)
{
    int newPercentage = (value * 100) / 127;
    if (newPercentage != potentiometerValue.getValue())
    {
        potentiometerValue.setValue(newPercentage);
    }
}
#endif

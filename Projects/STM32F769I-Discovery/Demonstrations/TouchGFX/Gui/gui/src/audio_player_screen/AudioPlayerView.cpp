...
void AudioPlayerView::setupScreen()
{
    ...
    blurBackground.setPosition(0, 0, 800, 480);
    ...
    prevButton.setXY(100 , (container.getHeight() - prevButton.getHeight()) / 2);
    ...
    nextButton.setXY(600, prevButton.getY());
    ...
    add(topBar);
    bottomBar.setY(480/* - bottomBar.getHeight()*/);
    add(bottomBar);
...
void AudioPlayerView::upscaleBlurredImage(uint8_t* src, int width, int height)
{
    ...
    for (int j = 0; j < BLUR_REGION_HEIGHT - 1; j++)
    {
        ...
        for (int k = 0; k < 4; k++)
        {
            ...
            {
                int rL = *src + k * (src[600] - src[0]) / 4;
                int gL = src[1] + k * (src[601] - src[1]) / 4;
                int bL = src[2] + k * (src[602] - src[2]) / 4;
                int rR = src[3] + k * (src[603] - src[3]) / 4;
                int gR = src[4] + k * (src[604] - src[4]) / 4;
                int bR = src[5] + k * (src[605] - src[5]) / 4;
...
#if defined(AUDIO_USE_SPIRIT_EQUALIZER)
    equalizer.setVisible(false);
#endif /* #ifdef AUDIO_USE_SPIRIT_EQUALIZER */
    showBackButton(false);

    if (presenter->isPlaylistEmpty())
    {
        //Keep displaying
        nextButton.setVisible(false);
        prevButton.setVisible(false);
...
void AudioPlayerView::progressSliderChanged(uint32_t newValue)
{
    ...
    uint32_t newProgress = (uint32_t)(100*newValue/360.0f);

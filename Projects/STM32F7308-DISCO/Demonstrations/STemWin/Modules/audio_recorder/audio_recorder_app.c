...
AUDIO_RECORDER_ErrorTypdef  AUDIO_RECORDER_Play(uint32_t frequency)
{
            ...
            (void *)&numOfReadBytes) == FR_OK)
  {
   ...
   SCB_CleanDCache_by_Addr((uint32_t*)&haudio.buff[0], AUDIO_OUT_BUFFER_SIZE);
    ...
    if( event.status == osEventMessage )
    {
      if(haudio.in.state == AUDIO_RECORDER_PLAYING)
      {
        ...
        case PLAY_BUFFER_OFFSET_HALF:
          ...
          {
            /* Clean Data Cache to update the content of the SRAM */
            SCB_CleanDCache_by_Addr((uint32_t*)&haudio.buff[0], AUDIO_OUT_BUFFER_SIZE/2);
        ...
        case PLAY_BUFFER_OFFSET_FULL:
          ...
          {
            /* Clean Data Cache to update the content of the SRAM */
            SCB_CleanDCache_by_Addr((uint32_t*)&haudio.buff[AUDIO_OUT_BUFFER_SIZE/2], AUDIO_OUT_BUFFER_SIZE/2);
      ...
      if(haudio.in.state == AUDIO_RECORDER_RECORDING)
      {
        ...
        case REC_BUFFER_OFFSET_HALF:
          ...
          SCB_InvalidateDCache_by_Addr((uint32_t *)(haudio.buff), AUDIO_IN_BUFFER_SIZE/2);
        ...
        case REC_BUFFER_OFFSET_FULL:
          /* Invalidate Data Cache to get the updated content of the SRAM */
          SCB_InvalidateDCache_by_Addr((uint32_t *)(haudio.buff + AUDIO_IN_BUFFER_SIZE/2), AUDIO_IN_BUFFER_SIZE/2);

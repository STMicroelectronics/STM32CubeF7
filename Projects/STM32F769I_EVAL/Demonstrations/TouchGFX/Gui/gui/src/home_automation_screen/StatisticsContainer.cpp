    ...
    : numValues(0), statState(STAT_STATE_NONE)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        graph[i].setPosition(80, 23, 690, 194);
        graph[i].setup(painter[i], bgPainter);
        graph[i].setRange(-10 * yScale, 110 * yScale);
        add(graph[i]);

        circle[i].setPosition(78 + 267 * i, 24 + upper.getHeight(), 110, 110);
        ...
        icon[i].setXY(18 + 267 * i, 109 + upper.getHeight());
        add(icon[i]);
        ...
        pctValue[i].setPosition(103 + 267 * i, 51 + upper.getHeight(), 60, pctValue[i].getTextHeight());
        pctValue[i].setColor(Color::getColorFrom24BitRGB(0xFF, 0xFF, 0xFF));
        add(pctValue[i]);
        pctSign[i].setTypedText(TypedText(T_PERCENT_SIGN));
        pctSign[i].setPosition(123 + 267 * i, 86 + upper.getHeight(), 20, pctSign[i].getTextHeight());

...
AudioFolderWidget::AudioFolderWidget(AudioPlaylistManager &manager)
    : selected(DIRLIST_LENGTH),
      playlistManager(manager),
      playlist(0),
      playlistLength(0),
      buttonCallback(this, &AudioFolderWidget::onButtonPressed),
      itemCallback(this, &AudioFolderWidget::onItemPressed)
{
    folderNameBuf[0] = 0;

    AudioFolderListElement::itemPressedAction = &itemCallback;
...
void AudioFolderWidget::onButtonPressed(const AbstractButton &button)
{
    if (&button == &okButton)
    {
...
void AudioFolderWidget::onItemPressed(const uint32_t index)
{
    //deselect
...
void AudioFolderWidget::setPlaylist(PlaylistElement *playlist, uint32_t length)
{
    this->playlist = playlist;
    this->playlistLength = length;
}

bool AudioFolderWidget::isOnPlaylist(const char *const foldername, fileinput::DirEntry &direntry)
{
...
void AudioFolderWidget::setupDirlist(const char *const foldername, fileinput::DirEntry *dirlist, uint32_t length)
{
...
GenericCallback< const uint32_t > *AudioFolderListElement::itemPressedAction = 0;

AudioFolderListElement::AudioFolderListElement()
{
    nameBuf[0] = 0;
    isSelected = false;
...
void AudioFolderListElement::setSelected(bool selected)
{
    isSelected = selected;
    if (selected)
    {
...
void AudioFolderListElement::handleClickEvent(const ClickEvent &event)
{
...
void AudioFolderListElement::configure(bool isDirectory)
{
    ...
    {
        icon.setXY(34, 8);
        icon.setBitmap(Bitmap(BITMAP_SONG_LIST_ICON_ID));
    }
    if (isIncluded)
    {
        button.setBitmap(Bitmap(BITMAP_SELECTED_ID));

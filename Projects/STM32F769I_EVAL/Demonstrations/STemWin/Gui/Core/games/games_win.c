...
/**
  * @brief  Paints Yes button
  * @param  hObj: button handle
  * @retval None
  */
static void _OnPaint_yes(BUTTON_Handle hObj) {
  ...
  GUI_DispStringAt("Yes", 70, 20);
}

/**
  * @brief  Paints No button
  * @param  hObj: button handle
  * @retval None
  */
static void _OnPaint_no(BUTTON_Handle hObj) {
...
static void _OnPaint_ok(BUTTON_Handle hObj) {
  ...
  GUI_SetFont(&GUI_FontLubalGraph24B);
  GUI_DispStringAt("Ok", 45, 25);
}
/**
  * @brief  callback for Yes button
  * @param  pMsg: pointer to data structure of type WM_MESSAGE
  * @retval None
  */
static void _cbButton_yes(WM_MESSAGE * pMsg) {
  switch (pMsg->MsgId) {
  case WM_PAINT:
    _OnPaint_yes(pMsg->hWin);
    break;
  default:
...
/**
  * @brief  callback for No button
  * @param  pMsg: pointer to data structure of type WM_MESSAGE
  * @retval None
  */
static void _cbButton_no(WM_MESSAGE * pMsg) {
  switch (pMsg->MsgId) {
  case WM_PAINT:
    _OnPaint_no(pMsg->hWin);
    break;
  default:
    /* The original callback */
    BUTTON_Callback(pMsg);
...
static int _ShowMessageBox(WM_HWIN hWin, const char* pTitle, const char* pText, int YesNo) {
  ...
  int r = 0;
  /* Create frame win */
  hFrame = FRAMEWIN_CreateEx(170, 120, 460, 200, hWin, WM_CF_SHOW, FRAMEWIN_CF_MOVEABLE, 0, pTitle, &_cbMessageBox);
  ...
  hItem = TEXT_CreateEx(10, 20, 450, 60, hClient, WM_CF_SHOW, GUI_TA_HCENTER, 0, pText);
  TEXT_SetFont(hItem, &GUI_Font24_ASCII);
  if (YesNo) {
    hBut = BUTTON_CreateEx(390, 100, 120, 120, hClient, WM_CF_SHOW, 0, GUI_ID_CANCEL);
    WM_SetCallback(hBut, _cbButton_no);
    //BUTTON_SetText        (hBut, "No");
    hBut = BUTTON_CreateEx((-60), 100, 120, 120, hClient, WM_CF_SHOW, 0, GUI_ID_OK);
    WM_SetCallback(hBut, _cbButton_yes);
    //BUTTON_SetText        (hBut, "Yes");
  } else {
    hBut = BUTTON_CreateEx(170, 100, 120, 120, hClient, WM_CF_SHOW, 0, GUI_ID_OK);
  ...
  WM_SetFocus(_hFrame);  
  WM_MakeModal(hFrame);
  r = k_ExecCreatedDialog(hFrame);//GUI_ExecCreatedDialog  
  return r;
}

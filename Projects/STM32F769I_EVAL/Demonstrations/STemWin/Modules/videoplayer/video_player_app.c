...
/** @defgroup VIDEO_APPLICATION
  * @brief video application routines
  * @{
  */
  ...
  for(;;)
  {
    ...
    if( event.status == osEventMessage )
    {
      ...
      case DECODE_CONVERT_OUTPUT_BUFF:
        MCU_BlockIndex += pConvert_Function( (uint8_t *)MCU_Data_OutBuffer, (uint8_t *)0xC0C00000, MCU_BlockIndex, CHUNK_SIZE_OUT, &ConvertedDataCount);    
        
        if((MCU_BlockIndex == MCU_TotalNb) && (MCU_TotalNb != 0))
        {
            LCD_LL_DrawBitmap16bpp(0, 0, 0, (uint16_t *)0xC0C00000, 800, 480, 1600);
        }
        else
        {

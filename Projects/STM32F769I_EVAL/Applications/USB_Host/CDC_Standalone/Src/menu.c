...
void Menu_Init(void)
{
  ...
  BSP_LCD_DisplayStringAtLine(23, (uint8_t *)"Use [Joystick Left/Right] to scroll up/down");
  BSP_LCD_DisplayStringAtLine(24, (uint8_t *)"Use [Joystick Up/Down] to scroll CDC menu");
...
void CDC_SelectItem(uint8_t **menu, uint8_t item)
{
  ...
  case 3:
    BSP_LCD_SetBackColor(LCD_COLOR_BLUE);
    BSP_LCD_DisplayStringAtLine(26, menu[1]);
    BSP_LCD_DisplayStringAtLine(27, menu[2]);
    BSP_LCD_SetBackColor(LCD_COLOR_MAGENTA);
    BSP_LCD_DisplayStringAtLine(28, menu[3]);
    break;

  case 4:
    BSP_LCD_SetBackColor(LCD_COLOR_BLUE);
    BSP_LCD_DisplayStringAtLine(26, menu[2]);
    BSP_LCD_DisplayStringAtLine(27, menu[3]);
    BSP_LCD_SetBackColor(LCD_COLOR_MAGENTA);
    BSP_LCD_DisplayStringAtLine(28, menu[4]);
    break;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  else if(GPIO_Pin == TAMPER_BUTTON_PIN)
  {
    ...
    else if (CdcDemo.state == CDC_DEMO_SEND)
    {
      /* Prevent debounce effect for user key */
      if((HAL_GetTick() - debounce_time) > 50)
      {

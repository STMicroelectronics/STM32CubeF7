...
void SerialUpload(void)
{
  ...
  if ( status == CRC16)
  {
    ...
    status = Ymodem_Transmit((uint8_t*)APPLICATION_ADDRESS, (const uint8_t*)"UploadedFlashImage_DualBankConf.bin", USER_FLASH_SIZE);
#else  /* Use a Single Bank Flash */
    /* Transmit the flash image through ymodem protocol */
    status = Ymodem_Transmit((uint8_t*)APPLICATION_ADDRESS, (const uint8_t*)"UploadedFlashImage_SingleBankConf.bin", USER_FLASH_SIZE);   

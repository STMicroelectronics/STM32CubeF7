...
static void CAMERA_Capture(void)
{
  while(1)
  {
    ...
    else if(BSP_PB_GetState(BUTTON_TAMPER) == GPIO_PIN_RESET)
    {
      HAL_Delay(10);
...
static void PicturePrepare(void)
{
  ...
  hdma2d_eval.Init.ColorMode    = DMA2D_OUTPUT_RGB888; /* Output color mode at the output of DMA2D operation */
...
static uint8_t LCD_Init(void)
{
  ...
  */
  ...
  /* Configure DSI_TE pin : Tearing effect on separated GPIO from KoD LCD             */
